...
EM  =  [sum(OXD(:,ind1),2) sum(OXD(:,ind2),2) sum(OXD(:,ind3),2)];
...
Ea_EM = [11.8739; -1.6950;   3.9259];
A0_EM = [-2.1207; -1.3624; -68.7967];

Ea = exp(Ea_EM(1) + Ea_EM(2).*EM(:,1) + Ea_EM(3).*EM(:,2));
A0 = exp(A0_EM(1) + A0_EM(2).*EM(:,1) + A0_EM(3).*EM(:,2));

etaf = A0 .* exp(Ea./R./(TC+273.15)); %[Pas]
end
...
switch init_mode
    ...
    case 'chamber'
        % Compute dimensionless distance r; r = 1 corresponds to the ellipse boundary.
        r = sqrt( ((XX - L/2)/(chmbw/2)).^2 + ((ZZ - D/2)/(chmbh/2)).^2 );

        % Approximate physical signed distance d from the ellipse boundary.
        d = (1 - r) * min(chmbw/2, chmbh/2);

        % Define a blending function using the error function.
        blend = 0.5 * (1 + erf(d/wlay_T));

        % Set up the temperature field: T1 inside the ellipse and T0 outside, with smooth blending.
        Tp = T0 + (T1 - T0) .* blend + dTr.*rp + dTg.*gp;

        % Define a blending function using the error function.
        blend = 0.5 * (1 + erf(d/wlay_c));

        % Set up the composition field: c1 inside the ellipse and c0 outside, with smooth blending.
        c = zeros(Nz,Nx,cal.ncmp);
        for i = 1:cal.ncmp
            c(:,:,i) = c0(i) + (c1(i) - c0(i)) .* blend + dcr(i).*rp + dcg(i).*gp;
        end

        % Set up the tracer field: trc1 inside the ellipse and trc0 outside, with smooth blending.
        trc = zeros(Nz,Nx,cal.ntrc);
        for i = 1:cal.ntrc
            trc(:,:,i) = trc0(i) + (trc1(i) - trc0(i)) .* blend + dr_trc(i).*rp + dg_trc(i).*gp;
        end
    case 'read_1D'

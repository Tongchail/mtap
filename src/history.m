...
dsumSdt =  sum(sum(bnd_S*h*h*1)) + sum(sum(diss_h*h*h*1)) ...
        +  sum(qz_advn_S(1,2:end-1)*h*1) - sum(qz_advn_S(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_S(1,2:end-1)*h*1) - sum(qz_dffn_S(end,2:end-1)*h*1) ...
        + (sum(qx_advn_S(2:end-1,1)*h*1) - sum(qx_advn_S(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_S(2:end-1,1)*h*1) - sum(qx_dffn_S(2:end-1,end)*h*1))*(1-periodic);  % [J/K/s]
dsumBdt =  sum(qz_advn_X(1,2:end-1)*h*1) - sum(qz_advn_X(end,2:end-1)*h*1) ...
        ...
        +  sum(qz_advn_F(1,2:end-1)*h*1) - sum(qz_advn_F(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_X(1,2:end-1)*h*1) - sum(qz_dffn_X(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_M(1,2:end-1)*h*1) - sum(qz_dffn_M(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_F(1,2:end-1)*h*1) - sum(qz_dffn_F(end,2:end-1)*h*1) ...
        + (sum(qx_advn_X(2:end-1,1)*h*1) - sum(qx_advn_X(2:end-1,end)*h*1) ...
        +  sum(qx_advn_M(2:end-1,1)*h*1) - sum(qx_advn_M(2:end-1,end)*h*1) ...
        +  sum(qx_advn_F(2:end-1,1)*h*1) - sum(qx_advn_F(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_X(2:end-1,1)*h*1) - sum(qx_dffn_X(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_M(2:end-1,1)*h*1) - sum(qx_dffn_M(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_F(2:end-1,1)*h*1) - sum(qx_dffn_F(2:end-1,end)*h*1))*(1-periodic);  % [kg/s]
dsumMdt =  sum(sum(Gm*h*h*1)) ...
        +  sum(qz_advn_M(1,2:end-1)*h*1) - sum(qz_advn_M(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_M(1,2:end-1)*h*1) - sum(qz_dffn_M(end,2:end-1)*h*1) ...
        + (sum(qx_advn_M(2:end-1,1)*h*1) - sum(qx_advn_M(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_M(2:end-1,1)*h*1) - sum(qx_dffn_M(2:end-1,end)*h*1))*(1-periodic);  % [kg/s]
dsumXdt =  sum(sum(Gx*h*h*1)) ...
        +  sum(qz_advn_X(1,2:end-1)*h*1) - sum(qz_advn_X(end,2:end-1)*h*1) ...
        +  sum(qz_dffn_X(1,2:end-1)*h*1) - sum(qz_dffn_X(end,2:end-1)*h*1) ...
        + (sum(qx_advn_X(2:end-1,1)*h*1) - sum(qx_advn_X(2:end-1,end)*h*1) ...
        +  sum(qx_dffn_X(2:end-1,1)*h*1) - sum(qx_dffn_X(2:end-1,end)*h*1))*(1-periodic);  % [kg/s]
...
dsumCdt =  squeeze(sum(sum(bnd_C*h*h*1,1),2) ...
        +  sum(qz_advn_C(1,2:end-1,:)*h*1,2) - sum(qz_advn_C(end,2:end-1,:)*h*1,2) ...
        +  sum(qz_dffn_C(1,2:end-1,:)*h*1,2) - sum(qz_dffn_C(end,2:end-1,:)*h*1,2) ...
        + (sum(qx_advn_C(2:end-1,1,:)*h*1,1) - sum(qx_advn_C(2:end-1,end,:)*h*1,1) ...
        +  sum(qx_dffn_C(2:end-1,1,:)*h*1,1) - sum(qx_dffn_C(2:end-1,end,:)*h*1,1))*(1-periodic)).';  % [kg/s]
dsumTdt =  squeeze(sum(sum(bnd_TRC*h*h*1,1),2) ...
        +  sum(qz_advn_TRC(1,2:end-1,:)*h*1,2) - sum(qz_advn_TRC(end,2:end-1,:)*h*1,2) ...
        +  sum(qz_dffn_TRC(1,2:end-1,:)*h*1,2) - sum(qz_dffn_TRC(end,2:end-1,:)*h*1,2) ...
        + (sum(qx_advn_TRC(2:end-1,1,:)*h*1,1) - sum(qx_advn_TRC(2:end-1,end,:)*h*1,1) ...
        +  sum(qx_dffn_TRC(2:end-1,1,:)*h*1,1) - sum(qx_dffn_TRC(2:end-1,end,:)*h*1,1))*(1-periodic)).';  % [kg/s]

if step>=2; hist.DS(stp  ) = (a2*hist.DS(max(1,stp-1)  ) + a3*hist.DS(max(1,stp-2)  ) + (b1*dsumSdt + b2*dsumSdto + b3*dsumSdtoo)*dt)/a1; else; hist.DS(stp  ) = 0; end  % [J/K]

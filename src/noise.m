...
sgf_raw = Xi * sqrt(phi.*fReL.*ke./taue .* (L0h./(L0h+h)).^3);    % eddy fluid noise speed
sgsx_raw = Xi * sqrt(chi.*      ks_x./tausx .* (l0h./(l0h+h)).^3);    % crystal segregation noise speed
sgsf_raw = Xi * sqrt(phi.*      ks_f./tausf .* (l0h./(l0h+h)).^3);    % fluid segregation noise speed
...
sgf_padL0 = zeros(Nz+padL0,Nx);
sgsx_padl0 = zeros(Nz+padl0,Nx);
sgsf_padl0 = zeros(Nz+padl0,Nx);
...
sgf_padL0(1+padL0/2:end-padL0/2,:) = sgf_raw;
sgf_padL0(            1:padL0/2,:) = repmat(sgf_raw(1  ,:),padL0/2,1);
sgf_padL0(end-padL0/2+1:end    ,:) = repmat(sgf_raw(end,:),padL0/2,1);

sgsx_padl0(1+padl0/2:end-padl0/2,:) = sgsx_raw;
sgsx_padl0(            1:padl0/2,:) = repmat(sgsx_raw(1  ,:),padl0/2,1);
sgsx_padl0(end-padl0/2+1:end    ,:) = repmat(sgsx_raw(end,:),padl0/2,1);

sgsf_padl0(1+padl0/2:end-padl0/2,:) = sgsf_raw;
sgsf_padl0(            1:padl0/2,:) = repmat(sgsf_raw(1  ,:),padl0/2,1);
sgsf_padl0(end-padl0/2+1:end    ,:) = repmat(sgsf_raw(end,:),padl0/2,1);

sge_padL0 = real(ifft2(Gkpe_padL0 .* fft2(sge_padL0)));
sgx_padL0 = real(ifft2(Gkpe_padL0 .* fft2(sgx_padL0)));
sgf_padL0 = real(ifft2(Gkpe_padL0 .* fft2(sgf_padL0)));
sgsx_padl0 = real(ifft2(Gkps_padl0 .* fft2(sgsx_padl0)));
sgsf_padl0 = real(ifft2(Gkps_padl0 .* fft2(sgsf_padl0)));

sge = sge_padL0(1+padL0/2:end-padL0/2,:);
sgx = sgx_padL0(1+padL0/2:end-padL0/2,:);
sgf = sgf_padL0(1+padL0/2:end-padL0/2,:);
sgsx = sgsx_padl0(1+padl0/2:end-padl0/2,:); 
sgsf = sgsf_padl0(1+padl0/2:end-padl0/2,:); 
...
sgf = (sgf-mean(sgf(:))).*std(sgf_raw(:))./std(sgf(:)) + mean(sgf_raw(:));
sgsx = (sgsx-mean(sgsx(:))).*std(sgsx_raw(:))./std(sgsx(:)) + mean(sgsx_raw(:));
sgsf = (sgsf-mean(sgsf(:))).*std(sgsf_raw(:))./std(sgsf(:)) + mean(sgsf_raw(:));
...
psif = Fte .* psifo + sqrt(1 - Fte.^2) .* sgf.*fL .* rx;      % update particle eddy noise potential
psisx = Ftsx .* psisxo + sqrt(1 - Ftsx.^2) .* sgsx.*fl .* rs;      % update particle settling noise potential
psisf = Ftsf .* psisfo + sqrt(1 - Ftsf.^2) .* sgsf.*fl .* rs;      % update particle settling noise potential
